...
impl GTP {
    ...
    // Accepts individual command strings and returns the DEPRECATED_engine response from the command as a String
...
impl GTP {
    ...
    fn version(&self) -> GtpResponse {
        GtpResponse::SUCCESS("1".to_string())
    }

    // args[0] = command name to check
    // Checks if the given command name is a command this DEPRECATED_engine supports
    ...
    // TODO: IMPLEMENT
    // args[0] = Color ("B", "W")
    // Attempts to generate an DEPRECATED_engine move for the given color in the current Board position
    // Outputs the intersection to play at in Go Notation, "pass" if the DEPRECATED_engine wishes to pass,
    // or "resign" if the DEPRECATED_engine is resigning
